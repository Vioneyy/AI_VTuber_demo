...
class VTubeStudioController:
    ...
    def __init__(self):
        ...
        # แจ้งเตือน reconnect ครั้งเดียวเพื่อลดสแปมใน terminal
        self._reconnect_logged_once = False
        # แจ้งครั้งเดียวสำหรับ Auth / Model / Parameters
        self._auth_logged_once = False
        self._model_logged_once = False
        self._params_logged_once = False
        ...
        # ลดความผันผวนของความแรงเพื่อให้การเคลื่อนไหวนุ่มนวลขึ้น
        self.intensity_variation = 0.15
        ...
        try:
            # กำหนด floor ขั้นต่ำ ~22 FPS เพื่อกันการส่งถี่เกินไป
            self._min_send_interval: float = max(0.045, float(getattr(config.vtube, "send_min_interval_ms", 32)) / 1000.0)
        except Exception:
    ...
    async def _ensure_ws(self) -> bool:
        ...
        try:
            ...
            if not self.model_loaded:
                ...
                return False
            await self._get_available_parameters()
            # แจ้งครั้งแรกเป็น INFO ครั้งถัดไปลดลงเป็น DEBUG
            if not self._reconnect_logged_once:
                logger.info("✅ Reconnected VTS WebSocket")
                self._reconnect_logged_once = True
            else:
                logger.debug("✅ Reconnected VTS WebSocket")
            self._reconnecting = False
    ...
    async def _authenticate(self):
        ...
        try:
            ...
            if "authenticationToken" in response.get("data", {}):
                ...
                if not self._auth_logged_once:
                    logger.info("✅ VTS Authentication สำเร็จ")
                    self._auth_logged_once = True
                else:
                    logger.debug("✅ VTS Authentication สำเร็จ")
            else:
    ...
    async def _get_current_model(self):
        ...
        try:
            ...
            if response.get("data", {}).get("modelLoaded"):
                ...
                if not self._model_logged_once:
                    logger.info(f"✅ พบโมเดล: {model_name}")
                    self._model_logged_once = True
                else:
                    logger.debug(f"✅ พบโมเดล: {model_name}")
            else:
    ...
    async def _get_available_parameters(self):
        ...
        try:
            ...
            if response.get("data", {}).get("defaultParameters"):
                ...
                if not self._params_logged_once:
                    logger.info(f"📋 พบ {len(self.available_parameters)} parameters")
                else:
                    logger.debug(f"📋 พบ {len(self.available_parameters)} parameters")
                ...
                if available_important:
                    if not self._params_logged_once:
                        ...
                        self._params_logged_once = True
                    else:
                        logger.debug(f"✅ Parameters ที่มี: {', '.join(available_important)}")
                else:
    ...
    async def _send_parameters(self, parameters: Dict[str, float]):
        ...
        try:
            valid_params = []
            # ✅ Delta guard: ส่งเฉพาะพารามิเตอร์ที่เปลี่ยนมากพอ (เพิ่ม threshold เพื่อลด jitter)
    ...
    def _generate_random_movement(self) -> Dict[str, float]:
        ...
        # ลดความแรงพื้นฐานลงเล็กน้อยเพื่อลดการกระตุกระหว่าง idle
        base_intensity = random.uniform(0.5, 0.85)
            ...
            try:
                ...
                if not (self.state == AnimationState.IDLE and not config.vtube.idle_motion_enabled):
                    if current_time - self.last_eye_movement >= self.eye_movement_duration:
                        ...
                        # ชะลอความถี่การขยับตาเพื่อให้ภาพนิ่งขึ้น
                        self.eye_movement_duration = random.uniform(1.2, 2.4)
            ...
            try:
                ...
                for i in range(0, len(audio), chunk_size):
                    ...
                    if abs(mouth_open - last_mouth_value) > 0.02:
                        ...
                        # ✅ MouthForm: ปรับตามคอนฟิก (ค่าเริ่มต้นคือปิด variation เพื่อลด jitter)
                        if 'MouthForm' in self.available_parameters:
                            if getattr(config.vtube, 'lipsync_enable_mouthform_variation', False):
    ...
    async def stop_speaking(self):
        """หยุดพูด (ปิดปากแบบค่อยๆ และรีเซ็ต idle timers)"""
        logger.info("🛑 Stop speaking")
        self._lip_sync_running = False

        # ยกเลิกงาน lip sync หากยังทำงานอยู่
        if self._lip_sync_task and not self._lip_sync_task.done():
            try:
                self._lip_sync_task.cancel()
                await self._lip_sync_task
        ...
        # เคลียร์ MouthForm ด้วย
        if 'MouthForm' in self.available_parameters:
            await self.set_parameter_value('MouthForm', 0.0, immediate=False)

        # กลับสู่ idle และรีเซ็ตตัวตั้งเวลาเพื่อให้ idle motion กลับมาทันที
        self.state = AnimationState.IDLE
        self.last_movement_change = 0.0
        self.last_eye_movement = 0.0
        self.movement_duration = random.uniform(1.0, 2.0) / max(1e-6, self.movement_speed)
    ...
    async def set_talking(self, talking: bool):
        """ตั้งสถานะกำลังพูด / หยุดพูด โดยจัดการ transition ให้เนียน"""
        if talking:
